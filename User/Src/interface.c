...
extern uint8_t threshold[THRESHOLD_ROW_SIZE][THRESHOLD_COLUMN_SIZE];
	...
	if (typeconnect == USB) {
		...
		if (isCalibaseMode) {
			if (data[0] == CMD_CALIBASE_SET_TYPE) { // Calibase Code: 'C' + 'Type'
				...
				if (data[1] == CMD_CALIBASE_WRITE_THRESHOLD) {
					//ThresholdWrite(); tạm thời ko dùng
					//isWriteThreshold = true;
				}
			}
		}
